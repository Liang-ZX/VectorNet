...
class SubgraphNet_Layer(nn.Module):
    ...
    def forward(self, input):
        ...
        maxpool = nn.MaxPool1d(kernel_size)  # max pool                # 最大值池化
...
class SubgraphNet(nn.Module):
    ...
    def forward(self, input):
        out1 = self.sublayer1(input) # 调用SubgraphNet_Layer.forward(input)， input.shape -> torch.Size([30, 6]) torch.Size([18, 8]) 
